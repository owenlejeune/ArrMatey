...
class BazarrClientImpl(
    ...
    override suspend fun getSystemStatus(): NetworkResult<BazarrSystemStatus> = httpClient.safeGet("$baseUrl/system/status")

    override suspend fun getSystemSettings(): NetworkResult<BazarrSystem> = httpClient.safeGet("$baseUrl/system/settings")

    override suspend fun getBadges(): NetworkResult<BazarrBadges> = get("badges")

    override suspend fun getWantedEpisodes(): NetworkResult<WantedEpisodesResponse> = get("episodes/wanted")
    ...
    override suspend fun getSeries(): NetworkResult<List<BazarrSeries>> = get<BazarrSeriesResponse>("series").map { it.data }

    override suspend fun getMovies(): NetworkResult<List<BazarrMovie>> = get<BazarrMoviesResponse>("movies").map { it.data }

    override suspend fun getProviders(): NetworkResult<ProvidersResponse> = get("providers")
    ...
    override suspend fun getSubtitleFile(
        path: String,
        onProgress: (Float) -> Unit,
    ): NetworkResult<ByteArray> = get("subtitles/contents", mapOf("subtitlePath" to path), onProgress = onProgress)
    ...
    private suspend inline fun <reified T> post(
        ...
        params: Map<String, Any> = emptyMap(),
        timeoutMillis: Long? = null,
    ): NetworkResult<T> = httpClient.safePost<T>("$baseUrl/$endpoint") { applyParams(params, timeoutMillis) }
