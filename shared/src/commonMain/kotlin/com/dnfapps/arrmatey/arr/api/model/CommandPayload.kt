...
sealed class CommandPayload(val name: String) {
    ...
    @Serializable
    data class RefreshSeries(val seriesIds: List<Long>? = null): CommandPayload("RefreshSeries")

    @Serializable
    data class RefreshMovie(val movieIds: List<Long>? = null): CommandPayload("RefreshMovie")

    @Serializable
    data class RefreshAlbum(val albumIds: List<Long>? = null): CommandPayload("RefreshAlbum")

    @Serializable
    data class RefreshArtist(val artistIds: List<Long>? = null): CommandPayload("RefreshArtist")

    @Serializable
    data class RefreshAuthor(val authorIds: List<Long>? = null): CommandPayload("RefreshAuthor")
