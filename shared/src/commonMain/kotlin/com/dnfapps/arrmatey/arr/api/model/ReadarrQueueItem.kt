...
data class ReadarrQueueItem(
    ...
    override val titleLabel: String
        get() {
            author?.title?.let { authorTitle ->
                book?.title?.let { bookTitle ->
                    return "$authorTitle - $bookTitle"
                }
                return authorTitle
            }
            return book?.title ?: title ?: "Unknown"
        }

    override val mediaId: Long?
        get() = authorId ?: author?.id ?: book?.authorId
}