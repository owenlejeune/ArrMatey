...
class ListenarrClient(
    ...
    override suspend fun getReleases(params: ReleaseParams): NetworkResult<List<ListenarrRelease>> {
        val query =
            (params as? ReleaseParams.Audiobook)?.query
    ...
    suspend fun getConfigurationSettings(): NetworkResult<ListenarrConfiguration> = get("configuration/settings")

    suspend fun getMetadata(
        asin: String,
        region: String,
    ): NetworkResult<AudiobookMetadataResponse> = get("metadata/$asin", mapOf("region" to region))

    suspend fun getPreviewPath(
        rootPath: String,
        metadata: AudiobookMetadataBody,
    ): NetworkResult<AudiobookPreviewPaths> = post("library/preview-path", PreviewPathBody(rootPath, metadata))
