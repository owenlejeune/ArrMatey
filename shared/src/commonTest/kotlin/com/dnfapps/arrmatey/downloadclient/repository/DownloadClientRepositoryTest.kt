...
class DownloadClientRepositoryTest {
    ...
    private val fakeDao = object : DownloadClientDao {
        ...
        override fun observeSelectedDownloadClient(): Flow<DownloadClient?> = 
            MutableStateFlow(clients.value.find { it.selected })

        override suspend fun getDownloadClientById(id: Long): DownloadClient? = 
            clients.value.find { it.id == id }
        ...
        override suspend fun findOtherByUrl(url: String, currentId: Long): Long? = 
            clients.value.find { it.url == url && it.id != currentId }?.id

        override suspend fun findOtherByLabel(label: String, currentId: Long): Long? = 
            clients.value.find { it.label == label && it.id != currentId }?.id
