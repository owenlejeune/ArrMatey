...
class UnifiedMediaDetailsViewModelTest {
    ...
    fun testResolvedTypes() {
        ...
        val viewModel = UnifiedMediaDetailsViewModel(
            arrId = 1L,
            tmdbId = 1L,
            tvdbId = 1L,
            instanceType = null,
            ...
            getArrInstanceRepositoryUseCase = mockk(relaxed = true) {
                every { observeSelected(any()) } returns flowOf(null)
            },
            getSeerrInstanceRepositoryUseCase = mockk(relaxed = true) {
                every { observeSelected() } returns flowOf(null)
            },
            getBazarrInstanceRepositoryUseCase = mockk(relaxed = true) {
                every { observeSelected() } returns flowOf(null)
            },
